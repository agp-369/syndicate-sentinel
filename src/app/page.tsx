...
import { ShieldCheck, ArrowRight, Loader2, Sparkles, Zap, Terminal, CheckCircle2, Database, Search, AlertTriangle, Construction, Bot, LogOut, Workflow, Fingerprint } from "lucide-react";
...
function SentinelContent() {
  ...
  const [log, setLog] = useState<string[]>(["Lumina OS v14.0 // Initializing..."]);
  ...
  useEffect(() => {
    ...
    if (effectiveToken && userId) {
      ...
      checkDiagnostics(effectiveToken);
    } else if (userId) {
      setStep("HANDSHAKE");
    }
  }, [searchParams, userId]);

  const checkDiagnostics = async (token: string) => {
    try {
      ...
      if (!data.gemini) setSystemAlert("WARNING: Gemini API Key missing in Environment.");
      if (!data.notion) setSystemAlert("WARNING: Notion Internal Token missing in Environment.");
    } catch (e) {
  ...
  const scanWorkspace = async (token: string) => {
    setIsSyncing(true);
    addLog("Scanning Notion for Infrastructure...");
    try {
      const res = await fetch("/api/sentinel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const initializeInfrastructure = async () => {
    setIsSyncing(true);
    addLog("Provisioning Sovereign Repositories...");
    try {
      const res = await fetch("/api/sentinel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const generateStrategy = async (employee: any) => {
    ...
    setIsSyncing(true);
    addLog(`The Oracle: Synthesizing Strategy for ${name}...`);
    try {
      const res = await fetch("/api/sentinel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleForensicScan = async () => {
    ...
    addLog(`Lumina Sentinel: Deep Scrape Initiated...`);
    ...
    try {
      ...
      if (data.success) {
        ...
        addLog(`HITL STAGE 1: Job Logged as AWAITING_REVIEW in Notion.`);
      } else {
        throw new Error(data.error || "Analysis Failed");
      }
    } catch (e: any) {
      addMcpLog("mcp/tool_error", { error: e.message });
  ...
  return (
    <div className="min-h-screen bg-[#0A0F1E] text-slate-300 font-mono p-4 md:p-8 flex flex-col items-center">
      ...
      {systemAlert && (
        <div className="fixed top-0 left-0 w-full bg-red-600 text-white text-[10px] py-1 font-bold z-[500] text-center uppercase tracking-widest">
            {systemAlert}
        </div>
      )}
      ...
      <div className="fixed bottom-8 right-8 z-[200] max-w-xs w-full bg-slate-900/90 backdrop-blur-xl p-6 rounded-3xl border border-white/10 shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Lumina // Monitor</p>
        </div>
      ...
      {step === "IDENTITY" && (
        <motion.div key="identity" className="flex flex-col items-center text-center space-y-12 mt-32 max-w-2xl relative z-10">
          <div className="w-24 h-24 bg-indigo-600 rounded-[2.5rem] flex items-center justify-center text-white shadow-2xl"><Sparkles size={48} /></div>
          <div className="space-y-4">
            <h1 className="text-7xl font-black tracking-tighter uppercase text-white">Project<br/>Lumina</h1>
            <p className="text-[10px] uppercase tracking-[0.5em] text-slate-500">Autonomous Career OS // MCP Core</p>
          </div>
          <SignInButton mode="modal"><button className="px-10 py-5 bg-white text-slate-950 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center gap-4 cursor-pointer hover:bg-indigo-50 transition-all">Initialize Identity <ArrowRight size={16} /></button></SignInButton>
        </motion.div>
      )}
      ...
      {step === "HANDSHAKE" && (
        <motion.div key="handshake" className="flex flex-col items-center text-center space-y-12 mt-32 max-w-2xl relative z-10">
          ...
          <div className="space-y-4 text-center">
            <h2 className="text-5xl font-black tracking-tighter uppercase text-white leading-none">MCP<br/>Handshake</h2>
            <p className="text-sm italic text-slate-400 max-w-md">Sync Lumina with your Notion Career Workspace.</p>
          </div>
        ...
        <div className="max-w-6xl w-full space-y-10 mt-12 mb-32">
          ...
          <header className="flex items-center justify-between bg-slate-900/50 p-6 rounded-3xl border border-white/5 backdrop-blur-md">
            <div className="flex items-center gap-6">
              <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/20"><Bot size={20} /></div>
              <div className="text-left">
                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-400 leading-none">The Alchemist</p>
                <p className="text-[10px] font-bold text-slate-500 mt-1 uppercase tracking-widest">{user?.firstName} // Stage_1_Analysis</p>
              </div>
              <div className="hidden md:flex items-center gap-2 bg-slate-950/50 px-4 py-2 rounded-full border border-white/5">
                 ...
                 <p className="text-[8px] font-black uppercase tracking-widest text-slate-500">Notion Link: {workspace?.connected ? "ACTIVE" : "PENDING"}</p>
              </div>
            </div>
          ...
          <main className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-12 grid grid-cols-1 lg:grid-cols-12 gap-8">
              <div className="lg:col-span-7 bg-slate-900/50 p-10 rounded-[4rem] border border-white/5 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-3xl font-black uppercase tracking-tighter text-white">Forensic Sentinel</h3>
                </div>
                  ...
                  {forensicResult && (
                    ...
                    >
                        ...
                        <div className="p-4 bg-slate-950/50 rounded-2xl border border-white/5">
                          <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-2">Audit Flags</p>
              ...
              <div className="lg:col-span-5 bg-slate-950/80 p-10 rounded-[4rem] border border-emerald-500/10 relative overflow-hidden flex flex-col">
                 <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-xl font-black uppercase tracking-tighter text-emerald-500/80 italic">The Oracle // Live</h3>
                 </div>
            ...
            <div className="md:col-span-5 space-y-8">
              <div className="bg-slate-900/50 p-8 rounded-[3rem] border border-white/5 text-left space-y-6">
                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-500">Infrastructure</p>
                <div className="space-y-4">
                ...
                {!workspace?.connected && (
                  <button onClick={initializeInfrastructure} className="w-full py-6 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-4 cursor-pointer hover:bg-indigo-500 active:scale-95 transition-all shadow-xl shadow-indigo-500/10">
                    <Construction size={16} fill="currentColor" /> Provision Workspace
                  </button>
                )}
              </div>

              <div className="bg-indigo-600/5 p-8 rounded-[3rem] border border-indigo-500/10 text-left space-y-4">
                <div className="flex items-center gap-3 text-indigo-400"><Fingerprint size={18} /><p className="text-[10px] font-black uppercase tracking-widest text-white">The Brain</p></div>
                <p className="text-[11px] leading-relaxed text-slate-400 italic">Lumina constructs its own environment inside Notion. Ensure one page is shared to begin Stage 1.</p>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {talentPool.length > 0 ? (
                  talentPool.map((employee: any) => (
                    <motion.div key={employee.id} className="p-8 bg-slate-800/30 rounded-3xl border border-white/5 flex items-center justify-between group hover:border-indigo-500/50 transition-all">
                      <div className="text-left">
                        ...
                        <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-2">{employee.properties?.Role?.select?.name || "Stage 1: Awaiting Review"}</p>
                      </div>
                      <button onClick={() => generateStrategy(employee)} className="p-5 bg-indigo-600 text-white rounded-2xl hover:brightness-110 transition-all cursor-pointer flex items-center gap-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-white">Review</p>
                  ...
                  <div className="py-32 opacity-20 text-center space-y-6">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.5em] leading-relaxed text-center">System Idle.<br/>Provision workspace to begin.</p>
                  </div>
                )}
              </div>
            </div>
          </main>
          <footer className="opacity-10 text-center"><p className="text-[9px] font-black uppercase tracking-[1em]">Lumina OS // Sovereign Sentinel v14.0</p></footer>
        </div>
      )}
    </div>
  );
